...
**M1 COMPLETE — verdict NULL at both surfaces, well-powered (2026-08-03).** D-M1 resolved to Option C: M1a ran exactly as pre-committed, then M1b as a labeled camouflage arm (JSON tool-result rendering, constant titles, k=4 off-theme filler docs), both at 20 pairs. Every model held **20/20 clean trials in both gated cells on both arms**; 240/240 calls ok, zero vague, zero confabulation; fidelity gate 288/288. DG was 0/20 everywhere except `qwen-2.5-7b` at the camouflaged adversarial cell (2/20; Newcombe +0.100 [−0.077, +0.301] — straddles 0, so the null stands). Per-model Wilson 95% upper bound on DG: **16.1%**.
