...
2. **The number is explicitly a lower bound.** Paper **§5.2 body**: *"Absolute DG rates at completing-Cy conditions are lower bounds for non-L1 models (CITs calibrated to **L1's pharmacological prior**); Cx protection ratios and profile assignments are within-model comparisons and calibration-independent."* Appendix C Table 8 states the same thing. (Table 2's *caption* makes the narrower claim *"**Peak DG%** for non-L1 models are lower bounds"* — peak is `absent × synthetic_Y`, a different cell, so the caption is not the right citation for this point.) So Qwen2.5-7B's true completing-Cy rate under its *own* calibrated stimuli is ≥ 14%, not 14%.
...
- **Contradiction (the paper's own, unresolved by us):** the §5.2 body (and Appendix C Table 8) calls completing-Cy absolutes lower bounds for non-L1 models, and the calibration model reads 34% (Fig. 6, RAG-4) vs 67.0% (Table 1, 10-tool) at the same cell despite a claimed <2pp schema effect. Both cut against treating 14% as a point value.
