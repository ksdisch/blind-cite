...
2. **The number is explicitly a lower bound.** Paper **§5.2 body**: *"Absolute DG rates at completing-Cy conditions are lower bounds for non-L1 models (CITs calibrated to **L1's pharmacological prior**); Cx protection ratios and profile assignments are within-model comparisons and calibration-independent."* Appendix C Table 8 carries the same sentence. (Table 2's *caption* makes the narrower claim *"**Peak DG%** for non-L1 models are lower bounds"* — peak is `absent × synthetic_Y`, a different cell, so the caption is not the right citation for this point.) So Qwen2.5-7B's true completing-Cy rate under its *own* calibrated stimuli is ≥ 14%, not 14%.
