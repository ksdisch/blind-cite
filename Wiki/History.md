...
- **Landed:** the D23 pre-registration executed end to end on branch `feat/m1c-extension` — PR #11's two pre-freeze follow-ups (F15/F16) first, then p01–p20 pinned as `data/corpus_m1.json` / `data/docs_m1.json`, the corpus appended to 80 pairs, `m1c.py` + `test_m1c.py`, a full dry-run, the `ping` price re-pin, smoke on both arms, both waves (240/240 ok, first pass), and `m1c.py verdict` run **once**
