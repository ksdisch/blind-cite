...
- **Headline (as finally established):** the paper's Appendix C publishes per-cell matrices for all 13 models, giving **Qwen2.5-7B at `absent × prior_completing` = 14%** — our exact model at our exact cell. Our camouflaged 2/20 is **consistent with** that (p = 0.455), so this is not a refutation, and at N=20/cell the study cannot resolve 14% from 0 [Fact — `Wiki/Paper-Mapping.md`] — see D16, D17 in `Decisions.md`
- **Correction in flight:** two intermediate headlines were published and withdrawn inside this one PR — "paper-contradicting for cheap models" (D12, premise false) and "prior-dependence, well-powered" (D13, sized against the wrong anchor). Both withdrawals are recorded rather than hidden; adversarial review caught the second [Fact — PR #10 review F1/F2]
