...
- **Headline:** the extension is sized by a real power calculation this time — combined N=80 per gated cell per surface makes every reachable outcome map to exactly one of three pre-committed direction-only templates, with the D21 caveats inline [Fact — `docs/M1C-BRIEF.md` D2/D4]
