...
## Next actions
1. **Plan the pre-registered, power-sized extension (D19, approved).** A dedicated Fable session designs it *with* Kyle before anything is built or spent: target N from a power calculation against the 14% floor, the optional-stopping guard, the analysis pre-commitment, and how it is reported **alongside** — never in place of — the original N=20 result. Est. **≈$0.06** including the corpus extension it needs.
