...
- `Wiki/Results.md`, `Wiki/Why-The-Null.md` — corrected. Note "well-powered" still stands in two **frozen/append-only** records by design — `Decisions.md` D10 and `Wiki/History.md`'s PR #9 entry — withdrawn by D17/D18/D22 rather than rewritten. `CLAUDE.md` is neither, and now carries its own inline retraction.
- `README.md`, `PROJECT.md`, `CLAUDE.md` — headline corrected to a hedged directional statement; no point comparison anywhere.
- `docs/M1-BRIEF.md` — a correction block placed at the **top** of the post-run addendum, above everything it retracts; the addendum text itself left as the record of what was concluded at the time.
