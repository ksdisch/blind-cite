...
- **Current milestone:** **M0 (fit-pilot) — COMPLETE, verdict FIT (2026-07-15).** All three subjects survive (grounding 12/12 at ceiling, zero vague/confab, 144/144 calls ok); fidelity gate 16/16; generator rejection 0/36; total spend ≈$0.009. **Headline: DG = 0/36 at the adversarial cell** — all engagement was refusal or explicit discrimination (K4 "right-reason" flag on all three models). M1 sizing from the measured funnel: 20 pairs. See docs/M0-BRIEF.md "Pilot outcome" + `data/m0_verdict.json`. **Next action: Kyle resolves D5/D-M1** at `docs/M1-BRIEF.md` (written 2026-08-03 — argues A run-as-pre-committed / B camouflage variant / C both sequenced; recommends C). **No paid call until D5 resolves.** On the pick: record it in Decisions.md, append a brief addendum sweeping the review follow-ups from PR #8's review comment (headline bound worded "≤16.1%", the N≥20 top-up policy pre-committed before any paid wave, History merge-SHA backfill, README "days old"), then pre-commit `m1.py` + gates, dry-run, smoke, run.
