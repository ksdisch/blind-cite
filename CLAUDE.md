...
- **Current milestone:** **M0 (fit-pilot) — COMPLETE, verdict FIT (2026-07-15).** All three subjects survive (grounding 12/12 at ceiling, zero vague/confab, 144/144 calls ok); fidelity gate 16/16; generator rejection 0/36; total spend ≈$0.009. **Headline: DG = 0/36 at the adversarial cell** — all engagement was refusal or explicit discrimination (K4 "right-reason" flag on all three models). M1 sizing from the measured funnel: 20 pairs. See docs/M0-BRIEF.md "Pilot outcome" + `data/m0_verdict.json`. **D5 resolved (2026-08-03): M1 runs as Option C** — M1a exactly as pre-committed, then the labeled camouflage arm M1b (`docs/M1-BRIEF.md` "The pick" addendum + Decisions.md D6). **Next action: build `m1.py` — pre-commit gates, dry-run on synthetic answers, `ping` + measured-rate check, smoke N≈5, M1a wave, verdict; then the M1b build** (M0 fixtures, seed-preserving corpus extension to 20 pairs, extended fidelity gate at 100% before its wave). Remaining review follow-up: backfill PR #8's merge SHA in Wiki/History.md at the next wiki touch.
