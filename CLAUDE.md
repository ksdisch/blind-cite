...
- **Current milestone:** **M0 and M1 both COMPLETE (2026-08-03).** M0 fit-pilot: verdict FIT, DG 0/36 at the adversarial cell, all three models K4 "right-reason", spend ≈$0.009. M1 ran as Option C at 20 pairs (Decisions.md D6): **gate verdict NULL at both surfaces** (D10 — its "well-powered" wording is **withdrawn**, D17/D18) — 20/20 clean per gated cell per model on both arms, 240/240 calls ok, zero vague/confab, fidelity 288/288; DG 0/20 everywhere except `qwen-2.5-7b` camouflaged (2/20, CI straddles 0). **The flagship blindness contrast rendered** on those 2 answers (faithfulness PASS 2/2, citation PASS 2/2). DG-any 0/120 under fillers. M1 spend $0.0177/$0.45. See `docs/M1-BRIEF.md` "M1 outcome" addendum, `Wiki/Results.md`, `data/m1{a,b}_verdict.json`. **M1 gate verdict NULL — and no point comparison to the paper is legitimate (D21).** The paper has **no cell for the condition we ran**: §4/Appendix A define `prior_completing` by evidence elicited to match a model's prior for X, and ours is fabricated. The nearest published cell (Qwen2.5-7B, `absent × prior_completing`, RAG-4) reads **14%**, which paper §5.2 / Appendix C Table 8 call a **lower bound** for non-L1 models — and we ran neither of its schemas. Report **direction only**, hedged. Three headlines have already been withdrawn here (D12→D16, D13→D17, D17→D21), each failing the same way: comparing our rate to a cell that isn't ours. Underpowered too (D18): 0/20's Wilson upper is 16.1%, above the floor; 0/24 would clear it; the pre-committed N came from clean-trial yield, not a power calculation. See `Wiki/Paper-Mapping.md`. **Next action: build M1C per the committed pre-registration** (`docs/M1C-BRIEF.md`, D23) — on Kyle's explicit go, never before. Combined N=80 per gated cell per surface on `qwen-2.5-7b`, both surfaces, one-look verdict; est. **≈$0.052** under `CAP_M1C_TOTAL = $0.10`.
